...
export class SystemSubscriptionService {
  ...
  private async materializeEntitlements(subscriptionId: string, features: Record<string, unknown>) {
    await Promise.all(Object.entries(features).map(([key, value]) => this.db.entitlement.findOneAndUpdate(
      { subscriptionId, key },
      { $set: { value } },
  ...
  async assign(tenantId: string, planId: string, status = 'active', endsAt?: string, actorUserId?: string) {
    ...
    await this.materializeEntitlements(String(subscription._id), (plan as any).features ?? {});
