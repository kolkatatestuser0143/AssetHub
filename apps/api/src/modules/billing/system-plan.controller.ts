...
function themePreset(value: unknown): ThemePreset {
  ...
  if (!(THEME_PRESETS as readonly string[]).includes(normalized)) throw new BadRequestException('Invalid theme preset');
...
export class SystemPlanController {
  ...
  create(@Body() body: { name: string; features?: Record<string, unknown>; themePreset?: string }) { return this.subscriptions.createPlan(body.name, body.features ?? {}, themePreset(body.themePreset ?? 'starter')); }

  @Patch(':planId') @SystemPermission('platform:billing:manage')
  async update(@Param('planId') planId: string, @Body() body: { name: string; features?: Record<string, unknown>; themePreset?: string }) { const plan = await this.subscriptions.updatePlan(planId, body.name, body.features ?? {}, themePreset(body.themePreset ?? 'starter')); const entitlementSync = await this.entitlementSync.syncPlan(plan.id, plan.features ?? {}); return { ...plan, entitlementSync }; }
