...
export class SystemAdminService {
  ...
  async tenants() {
    ...
    for (const user of users as any[]) {
      ...
      if (!admins.has(key)) admins.set(key, user);
    }
    return tenants.map((tenant: any) => {
      const sub: any = byTenant.get(String(tenant._id));
      const admin: any = admins.get(String(tenant._id));
      return {
        id: String(tenant._id), name: tenant.name, slug: tenant.slug,
        primaryEmail: tenant.primaryEmail ?? admin?.email ?? null,
        ...
        admin: admin ? { id: String(admin._id), email: admin.email, name: `${admin.firstName ?? ''} ${admin.lastName ?? ''}`.trim(), isActive: admin.isActive !== false, forcePasswordReset: admin.forcePasswordReset === true } : null,
      };
    });
  }

  private generateTemporaryPassword() {
    return `Ah-${crypto.randomBytes(9).toString('base64url')}`;
  }
  ...
  async createTenant(input: { name: string; slug: string; email: string; planId?: string; actorUserId?: string }) {
    ...
    await this.db.user.create({
      tenantId: String(tenantId), companyId: String(companyId), accountType: UserAccountType.TENANT, email, passwordHash,
      firstName: name.split(/\s+/)[0] || 'Tenant', lastName: name.split(/\s+/).slice(1).join(' ') || 'Admin',
      isActive: true, forcePasswordReset: true, roleIds: [String(roleId)], backupCodesHash: [], createdAt: now, updatedAt: now,
    });
  ...
  async resetTenantPassword(tenantId: string, actorUserId?: string) {
    ...
    const user = await this.db.user.findOne({ tenantId, accountType: UserAccountType.TENANT }).sort({ createdAt: 1 }).lean();
    if (!user) throw new NotFoundException('Tenant administrator not found');
    const password = this.generateTemporaryPassword();
    const passwordHash = await argon2.hash(password, { type: argon2.argon2id });
  ...
  async tenantDetails(tenantId: string) {
    ...
    const [sub, users, companies] = await Promise.all([
      ...
      this.db.user.find({ tenantId, accountType: UserAccountType.TENANT }).select({ email: 1, firstName: 1, lastName: 1, isActive: 1, forcePasswordReset: 1 }).lean(),
      this.db.company.find({ tenantId }).lean(),
    ]);
