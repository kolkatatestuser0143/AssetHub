...
export class SystemSecurityService {
  ...
  async sessions() {
    ...
    return sessions.filter((s: any) => userMap.has(String(s.userId))).map((s: any) => {
      const user: any = userMap.get(String(s.userId));
      return {
        id: String(s._id), userId: String(s.userId), email: user.email,
        name: `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() || user.email,
        isActive: user.isActive !== false, ipAddress: s.ipAddress ?? null, userAgent: s.userAgent ?? null,
        createdAt: s.createdAt, lastSeenAt: s.lastSeenAt ?? s.createdAt, expiresAt: s.expiresAt,
      };
    });
  }
  ...
  async revokeSession(sessionId: string, actorUserId?: string) {
    ...
    if (result.modifiedCount > 0) await this.db.auditEvent.create({ actorUserId, action: 'security.session_revoked', targetType: 'session', targetId: sessionId, metadata: { userId: String(user._id), email: user.email }, result: 'success', occurredAt: new Date() });
  ...
  async revokeUserSessions(userId: string, actorUserId?: string, exceptSessionId?: string) {
    ...
    const user = await this.db.user.findOne({ _id: userId, accountType: UserAccountType.SYSTEM }).lean();
  ...
  async loginHistory() {
    ...
    const userMap = new Map(users.map((u: any) => [String(u._id), u]));
    const items = histories.filter((h: any) => userMap.has(String(h.userId))).map((h: any) => {
      const user: any = userMap.get(String(h.userId));
      return { id: String(h._id), userId: String(h.userId), email: user.email, name: `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() || user.email, success: h.success === true, reason: h.reason ?? null, ipAddress: h.ipAddress ?? null, userAgent: h.userAgent ?? null, occurredAt: h.occurredAt ?? h.createdAt };
    });
