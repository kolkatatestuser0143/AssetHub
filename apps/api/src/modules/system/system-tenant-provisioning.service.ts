...
export class SystemTenantProvisioningService {
  ...
  async createTenant(input: { name: string; slug: string; email: string; planId: string; actorUserId?: string }) {
    ...
    const tenant = await this.prisma.$transaction(async (tx) => {
      ...
      // Tenant Admin is tenant-wide. Keep companyId NULL so the role never becomes
      // accidentally restricted to the first company created during provisioning.
      await tx.$executeRawUnsafe(`INSERT INTO roles (id, tenant_id, company_id, name, is_system, created_at, updated_at) VALUES ($1::uuid,$2::uuid,NULL,$3,true,NOW(),NOW())`, roleId, tenant.id, 'Tenant Admin');
      ...
      const user = await tx.user.create({ data: { tenantId: tenant.id, companyId: company.id, accountType: 'TENANT', adminLevel: 'TENANT_ADMIN', email, passwordHash, firstName: name.split(/\s+/)[0] || 'Organization', lastName: name.split(/\s+/).slice(1).join(' ') || 'Admin', isActive: true, forcePasswordReset: true, authVersion: 0, roleIds: [roleId], backupCodesHash: [] } });
