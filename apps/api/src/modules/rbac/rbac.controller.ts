...
class UpdateRoleDto { @IsString() @MinLength(1) name!: string; @IsArray() @ArrayMinSize(0) permissionKeys!: string[]; }
class RoleScopeDto { @IsIn(['TENANT', 'COMPANY', 'LOCATION']) scopeType!: RoleScopeInput['scopeType']; @IsOptional() @IsString() companyId?: string; @IsOptional() @IsString() locationId?: string; }
class RoleScopesDto { @IsArray() scopes!: RoleScopeDto[]; }
...
export class RbacController {
  ...
  @Put(':roleId') @RequirePermission('role:write') updateRole(@Param('roleId') roleId:string,@Body() dto:UpdateRoleDto,@Req() req:any) { return this.rbac.updateRole(req.authContext,roleId,dto.name,dto.permissionKeys); }
