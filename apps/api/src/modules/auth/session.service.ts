...
export class SessionService {
  ...
  async issueSession(userId: string, ip: string, userAgent: string, system = false, familyId?: string, parentTokenHash?: string) {
    ...
    const access: TenantAccess = system ? { permissions: await this.resolveSystemPermissions(user.roleIds), crossCompany: false } : await this.resolveTenantAccess(user.tenantId, user.companyId, user.roleIds);
  ...
  async rotateRefreshToken(rawRefreshToken: string, ip: string, userAgent: string) { const hash = this.hashToken(rawRefreshToken); const existing = await this.prisma.session.findUnique({ where: { refreshTokenHash: hash } }); if (!existing) throw new UnauthorizedException('Invalid refresh token'); if (existing.revokedAt) { await this.revokeFamily(existing.familyId, 'refresh_token_reuse_detected'); throw new UnauthorizedException('Refresh token reuse detected'); } if (existing.expiresAt < new Date()) throw new UnauthorizedException('Refresh token expired'); const consumed = await this.prisma.session.updateMany({ where: { id: existing.id, refreshTokenHash: hash, revokedAt: null }, data: { revokedAt: new Date(), revokedReason: 'rotated' } }); if (!consumed.count) { await this.revokeFamily(existing.familyId, 'refresh_token_reuse_detected'); throw new UnauthorizedException('Refresh token reuse detected'); } return this.issueSession(existing.userId, ip, userAgent, await this.isSystemUser(existing.userId), existing.familyId, hash); }
  ...
  private async resolveTenantAccess(tenantId: string, companyId: string, roleIds: string[]): Promise<TenantAccess> { if (!roleIds.length) return { permissions: [], crossCompany: false }; const roles: any[] = await this.prisma.$queryRaw`SELECT "companyId", permissions FROM roles WHERE id = ANY(${roleIds}::uuid[]) AND "tenantId" = ${tenantId}::uuid AND ("companyId" = ${companyId}::uuid OR "companyId" IS NULL)`; const perms = new Set<string>(); let crossCompany = false; for (const role of roles) { if (role.companyId == null) crossCompany = true; for (const rp of (role.permissions ?? [])) perms.add(rp.permissionKey); } return { permissions: [...perms], crossCompany }; }
  private async resolveSystemPermissions(roleIds: string[]): Promise<string[]> { if (!roleIds.length) return []; const roles: any[] = await this.prisma.$queryRaw`SELECT permissions FROM roles WHERE id = ANY(${roleIds}::uuid[])`; const perms = new Set<string>(); for (const role of roles) for (const rp of (role.permissions ?? [])) perms.add(rp.permissionKey); return [...perms]; }
}
