...
export class AuthService {
  ...
  async login(email: string, password: string, ip: string, userAgent: string, tenantSlug?: string) {
    ...
    const source = user ? await this.getAuthSource(user.id) : 'LOCAL';
    if (source === 'SSO') throw new UnauthorizedException('This account uses single sign-on. Use your organization sign-in option.');
  ...
  private async getAuthSource(userId: string): Promise<'LOCAL' | 'SSO'> {
    const rows = await this.prisma.$queryRaw<Array<{ auth_source: string | null }>>`SELECT auth_source FROM users WHERE id = ${userId}::uuid LIMIT 1`;
  ...
  private async resolveSystemPermissions(roleIds: string[]): Promise<string[]> {
    ...
    const rows = await this.prisma.$queryRaw<Array<{ permission_key: string | null }>>`SELECT DISTINCT p.key AS permission_key FROM roles r INNER JOIN role_permissions rp ON rp.role_id = r.id INNER JOIN permissions p ON p.id = rp.permission_id WHERE r.id = ANY(${roleIds}::uuid[])`;
