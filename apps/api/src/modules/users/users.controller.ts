...
export class UsersController {
  ...
  @Put(':userId/roles') @RequirePermission('role:write') async updateRoles(@Param('userId') userId: string, @Body() dto: UpdateRolesDto, @Req() req: any) {
    ...
    const user = await this.db.user.findFirst({ where: { id: userId, tenantId: req.authContext.tenantId, accountType: 'TENANT', ...(req.authContext.crossCompany ? {} : { companyId: req.authContext.companyId }) } });
    if (!user) throw new NotFoundException('User not found');
    const uniqueRoleIds = [...new Set(dto.roleIds.map((id) => String(id).trim()).filter(Boolean))];
    const roles = uniqueRoleIds.length ? await this.db.$queryRawUnsafe<any[]>(`SELECT id, company_id AS "companyId" FROM roles WHERE tenant_id=$1::uuid AND id = ANY($2::uuid[])`, req.authContext.tenantId, uniqueRoleIds) : [];
    ...
    return this.db.user.update({ where: { id: userId }, data: { roleIds: uniqueRoleIds, authVersion: { increment: 1 } } });
  }
