...
export class UsersController {
 ...
 @Get(':userId/auth-source') @RequirePermission('user:read') authSource(@Param('userId') userId:string,@Req() req:any){return this.users.getAuthSource(req.authContext,userId);}
 ...
 @Patch(':userId/admin-level') @RequirePermission('user:write') changeAdminLevel(@Param('userId') userId:string,@Body() dto:AdminLevelDto,@Req() req:any){return this.users.changeAdminLevel(req.authContext,userId,dto.adminLevel,dto.authSource);}
 @Put(':userId/roles') @RequirePermission('role:write') async updateRoles(@Param('userId') userId:string,@Body() dto:UpdateRolesDto,@Req() req:any){if(String(userId)===String(req.authContext.userId))throw new ForbiddenException('You cannot modify your own roles. Ask another tenant administrator or the System Administrator.');const uniqueRoleIds=[...new Set(dto.roleIds.map(id=>String(id).trim()).filter(Boolean))];const result=await this.db.withTenantContext(req.authContext.tenantId,req.authContext.crossCompany?null:req.authContext.companyId,async tx=>{const user=await tx.user.findFirst({where:{id:userId,tenantId:req.authContext.tenantId,accountType:'TENANT'}});if(!user)throw new NotFoundException('User not found');const roles=uniqueRoleIds.length?await tx.$queryRawUnsafe<any[]>(`SELECT id,company_id AS "companyId" FROM roles WHERE tenant_id=$1::uuid AND id=ANY($2::uuid[]) AND (company_id IS NULL OR company_id=$3::uuid) AND NOT EXISTS(SELECT 1 FROM role_permissions rp JOIN permissions p ON p.id=rp.permission_id WHERE rp.role_id=roles.id AND p.key LIKE 'platform:%')`,req.authContext.tenantId,uniqueRoleIds,user.companyId):[];if(roles.length!==uniqueRoleIds.length)throw new NotFoundException('One or more roles are not available in this tenant');for(const role of roles){if(role.companyId!=null&&String(role.companyId)!==String(user.companyId))throw new ForbiddenException('Role belongs to a different company');if(role.companyId!=null&&!req.authContext.crossCompany&&String(role.companyId)!==String(req.authContext.companyId))throw new ForbiddenException('Role belongs to a different company');}return tx.user.update({where:{id:userId},data:{roleIds:uniqueRoleIds,authVersion:{increment:1}}});});return result;}
