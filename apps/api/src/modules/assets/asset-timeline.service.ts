...
export class AssetTimelineService extends TenantScopedRepository {
  ...
  async get(auth: AuthContext, assetId: string) {
    const asset = await this.db.asset.findOne({ _id: assetId, ...this.scope(auth) }).select({ _id: 1, assetNumber: 1, createdAt: 1 }).lean() as any;
