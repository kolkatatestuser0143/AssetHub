...
export class AssetTimelineService extends TenantScopedRepository {
  ...
  async get(auth: AuthContext, assetId: string) {
    ...
    const [lifecycle, assignments, documents, audits, transfers, maintenance, acknowledgements] = await Promise.all([
      ...
      this.db.assetAcknowledgement.find({ tenantId: auth.tenantId, companyId: auth.companyId, assetId }).sort({ generatedAt: -1 }).lean(),
    ]);

    const events: Array<Record<string, unknown>> = [{ type: 'asset.created', timestamp: (asset as any).createdAt ?? null, title: 'Asset created', description: `Asset ${asset.assetNumber} was created.`, source: 'asset', actorUserId: null }];
