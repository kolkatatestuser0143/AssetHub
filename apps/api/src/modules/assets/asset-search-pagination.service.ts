...
@Injectable()
export class AssetSearchPaginationService extends TenantScopedRepository {
  constructor(private readonly db: MongooseDatabaseService) { super(); }

  async list(
    auth: AuthContext,
    options: { q?: string; status?: string; assetTypeId?: string; page?: number; pageSize?: number },
  ) {
    ...
    const [total, rows] = await Promise.all([
      ...
      this.db.asset.find(filter).sort({ createdAt: -1, _id: -1 }).skip((page - 1) * pageSize).limit(pageSize).lean(),
    ]);
