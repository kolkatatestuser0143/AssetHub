...
export class AssetExcelReportService extends TenantScopedRepository {
  ...
  async generate(auth: AuthContext): Promise<Buffer> {
    await this.entitlements.requireFeature(auth.tenantId, 'advanced_reports_enabled');

    const assetScope = this.scope(auth);
    ...
    const headers = [
      'Asset Number', 'Status', 'Asset Type', 'Company', 'Vendor', 'Location', 'Department',
      'Assigned To', 'Warranty Provider', 'Warranty Expires', 'Created At', 'Asset ID',
    ];
    const rows = assets.map((asset: any) => {
      ...
      return [
        asset.assetNumber ?? '',
        asset.status ?? '',
        typeMap.get(String(asset.assetTypeId))?.name ?? '',
        companyMap.get(String(asset.companyId))?.name ?? '',
        vendorMap.get(String(asset.vendorId))?.name ?? '',
        locationMap.get(String(asset.locationId))?.name ?? '',
        departmentMap.get(String(asset.departmentId))?.name ?? '',
        assignedUser ? `${assignedUser.firstName ?? ''} ${assignedUser.lastName ?? ''}`.trim() || assignedUser.email : '',
        warranty?.provider ?? '',
        warranty?.expiresAt ? new Date(warranty.expiresAt).toISOString().slice(0, 10) : '',
    ...
    const sheet1 = this.sheetXml(summary, { freezeRows: 6, headerRow: 6 });
  ...
  private countBy(values: string[]): string[][] {
    const counts = new Map<string, number>();
    for (const value of values) counts.set(value, (counts.get(value) ?? 0) + 1);
    return [...counts.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([key, count]) => [key, String(count)]);
  }
  ...
  private sheetXml(rows: string[][], options: { freezeRows: number; headerRow: number; autoFilterEndColumn?: number; autoFilterEndRow?: number }) {
    const xmlRows = rows.map((row, rowIndex) => {
      const cells = row.map((value, colIndex) => {
        const ref = `${this.colName(colIndex + 1)}${rowIndex + 1}`;
        const style = rowIndex === options.headerRow - 1 ? 1 : (rowIndex === 0 && options.headerRow > 1 ? 2 : 0);
        return `<c r="${ref}" s="${style}" t="inlineStr"><is><t xml:space="preserve">${escapeXml(String(value ?? ''))}</t></is></c>`;
      }).join('');
      return `<row r="${rowIndex + 1}">${cells}</row>`;
    }).join('');
    const autoFilter = options.autoFilterEndColumn && options.autoFilterEndRow
      ? `<autoFilter ref="A${options.headerRow}:${this.colName(options.autoFilterEndColumn)}${options.autoFilterEndRow}"/>`
      : '';
