...
export class UploadcareDocumentStorage implements DocumentStorage {
  ...
  async register(key: string): Promise<StoredDocument> {
    this.assertConfigured();
    const normalized = String(key ?? '').trim();
    if (!/^[0-9a-f-]{20,}$/i.test(normalized)) throw new ServiceUnavailableException('Invalid Uploadcare file identifier');
    const response = await fetch(`${this.cdnBase}/${encodeURIComponent(normalized)}/`, { method: 'HEAD' });
    if (!response.ok) throw new ServiceUnavailableException(`Uploadcare file could not be verified (${response.status})`);
    return { key: normalized, provider: 'uploadcare', url: `${this.cdnBase}/${normalized}/` };
  }
