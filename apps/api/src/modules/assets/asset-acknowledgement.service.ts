...
export class AssetAcknowledgementService extends TenantScopedRepository {
  ...
  async deleteTemplate(auth: AuthContext, templateId: string) {
    const target = await this.db.assetAcknowledgementTemplate.findOne({ _id: templateId, tenantId: auth.tenantId }).lean();
    if (!target) throw new NotFoundException('Acknowledgement template not found');
    if (target.isDefault) throw new ForbiddenException('Default acknowledgement template cannot be deleted');
    await this.db.assetAcknowledgementTemplate.deleteOne({ _id: templateId, tenantId: auth.tenantId });
    return { ok: true };
  }

  async listAcknowledgements(auth: AuthContext, assetId: string) {
    const asset = await this.db.asset.findOne({ _id: assetId, ...this.scope(auth) }).select({ _id: 1 }).lean();
    if (!asset) throw new NotFoundException('Asset not found');
    return this.db.assetAcknowledgement.find({ tenantId: auth.tenantId, companyId: auth.companyId, assetId }).sort({ generatedAt: -1 }).lean();
  }

  async latest(auth: AuthContext, assetId: string) {
    const rows = await this.listAcknowledgements(auth, assetId);
    return rows[0] ?? null;
  }
  ...
  async generate(auth: AuthContext, assetId: string, templateId?: string): Promise<{ buffer: Buffer; acknowledgementId: string }> {
    ...
    const values: Record<string, string> = {
      '{{employee.name}}': `${employee.firstName} ${employee.lastName}`.trim(), '{{employee.employeeId}}': employee.employeeId ?? '', '{{employee.email}}': employee.email ?? '', '{{employee.jobTitle}}': employee.jobTitle ?? '',
      '{{asset.assetNumber}}': asset.assetNumber ?? '', '{{asset.type}}': assetType?.name ?? '', '{{asset.serialNumber}}': asset.serialNumber ?? '', '{{asset.model}}': asset.model ?? '',
      '{{assignment.date}}': assignment.assignedAt ? new Date(assignment.assignedAt).toLocaleDateString() : '', '{{issuer.name}}': issuer ? `${issuer.firstName} ${issuer.lastName}`.trim() : '',
    };
    const body = Object.entries(values).reduce((text, [token, value]) => text.split(token).join(value), template.content);
    const generatedAt = new Date();
    const record = await this.db.assetAcknowledgement.create({ tenantId: auth.tenantId, companyId: auth.companyId, assetId, employeeId: String(employee._id), templateId: String(template._id), templateName: template.name, generatedAt, generatedByUserId: auth.userId, status: 'PENDING' });
    return { buffer: buildPdf(body), acknowledgementId: String(record._id) };
  }

  async acknowledge(auth: AuthContext, acknowledgementId: string, note?: string) {
    const record = await this.db.assetAcknowledgement.findOne({ _id: acknowledgementId, tenantId: auth.tenantId, companyId: auth.companyId }).lean();
    if (!record) throw new NotFoundException('Acknowledgement not found');
    if (record.employeeId !== auth.userId) throw new ForbiddenException('Only the assigned employee can acknowledge this asset');
    if (record.status === 'ACKNOWLEDGED') return record;
    return this.db.assetAcknowledgement.findOneAndUpdate(
      { _id: acknowledgementId, tenantId: auth.tenantId, companyId: auth.companyId, status: 'PENDING' },
      { $set: { status: 'ACKNOWLEDGED', acknowledgedAt: new Date(), acknowledgedByUserId: auth.userId, acknowledgementNote: String(note ?? '').trim() || undefined } },
      { new: true },
    ).lean();
  }
}
...
function buildPdf(content: string): Buffer {
  const lines = content.split(/\r?\n/).flatMap((line) => wrap(line, 92)); const pageHeight = 842; const pageWidth = 595; const margin = 48; const lineHeight = 14; const maxLines = 50;
  const pages: string[][] = []; for (let i = 0; i < lines.length; i += maxLines) pages.push(lines.slice(i, i + maxLines)); if (!pages.length) pages.push(['']);
  const objects: string[] = ['']; objects.push('<< /Type /Catalog /Pages 2 0 R >>'); const fontObject = 3 + pages.length * 2; objects.push(`<< /Type /Pages /Kids [${pages.map((_, i) => `${4 + i * 2} 0 R`).join(' ')}] /Count ${pages.length} >>`);
  for (let i = 0; i < pages.length; i += 1) { const pageObj = 4 + i * 2; const contentObj = pageObj + 1; const commands = ['BT', '/F1 10 Tf', `${margin} ${pageHeight - 56} Td`]; pages[i].forEach((line, idx) => { if (idx > 0) commands.push(`0 -${lineHeight} Td`); commands.push(`(${pdfEscape(line)}) Tj`); }); commands.push('ET'); const stream = commands.join('\n'); objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${pageWidth} ${pageHeight}] /Resources << /Font << /F1 ${fontObject} 0 R >> >> /Contents ${contentObj} 0 R >>`); objects.push(`<< /Length ${Buffer.byteLength(stream, 'utf8')} >>\nstream\n${stream}\nendstream`); }
  objects.push('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>'); const chunks: Buffer[] = [Buffer.from('%PDF-1.4\n')]; const offsets: number[] = [0]; let offset = chunks[0].length; for (let i = 1; i < objects.length; i += 1) { offsets[i] = offset; const obj = Buffer.from(`${i} 0 obj\n${objects[i]}\nendobj\n`); chunks.push(obj); offset += obj.length; } const xrefOffset = offset; const xref = ['xref', `0 ${objects.length}`, '0000000000 65535 f ']; for (let i = 1; i < objects.length; i += 1) xref.push(`${String(offsets[i]).padStart(10, '0')} 00000 n `); xref.push('trailer', `<< /Size ${objects.length} /Root 1 0 R >>`, 'startxref', String(xrefOffset), '%%EOF'); chunks.push(Buffer.from(`${xref.join('\n')}\n`)); return Buffer.concat(chunks);
}
