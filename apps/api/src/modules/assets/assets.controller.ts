...
export class AssetsController {
  ...
  @Post() @RequirePermission('asset:write') async create(@Body() dto: CreateAssetDto, @Req() req: any) {
    ...
    if (dto.companyId) {
      ...
      if (!tenantWide && String(dto.companyId) !== String(req.authContext.companyId)) throw new ForbiddenException('Selected company is outside your scope');
      assetAuth = { ...req.authContext, companyId: String(dto.companyId), crossCompany: tenantWide };
    }
    if (dto.locationId) {
      const location = await this.db.location.findById(dto.locationId).lean(); if (!location) throw new NotFoundException('Location not found');
      const site = await this.db.plant.findById(location.plantId).lean(); if (!site) throw new NotFoundException('Site not found');
      const company = await this.db.company.findOne({ _id: site.companyId, tenantId: req.authContext.tenantId }).select({ _id: 1 }).lean(); if (!company) throw new ForbiddenException('Selected location does not belong to this tenant');
      ...
      if (tenantWide) assetAuth = { ...assetAuth, companyId: String(site.companyId), crossCompany: true };
    } else if (dto.departmentId) {
      const department = await this.db.department.findById(dto.departmentId).lean(); if (!department) throw new NotFoundException('Department not found');
      const location = await this.db.location.findById(department.locationId).lean(); const site = location ? await this.db.plant.findById(location.plantId).lean() : null; if (!site) throw new NotFoundException('Site not found');
      const company = await this.db.company.findOne({ _id: site.companyId, tenantId: req.authContext.tenantId }).select({ _id: 1 }).lean(); if (!company) throw new ForbiddenException('Selected department does not belong to this tenant');
