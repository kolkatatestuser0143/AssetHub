...
export class CustomFieldsService extends TenantScopedRepository {
  ...
  async createDefinition(auth: AuthContext, key: string, label: string, fieldType: string) {
    ...
    try {
      const doc = await this.db.customFieldDefinition.create({ tenantId: auth.tenantId, companyId: auth.companyId, key: normalizedKey, label: label.trim(), fieldType });
  ...
  async updateDefinition(auth: AuthContext, key: string, label?: string, fieldType?: string) {
    ...
    const updated = await this.db.customFieldDefinition.findOneAndUpdate({ ...this.scope(auth), key }, { ...(label !== undefined ? { $set: { label: label.trim() } } : {}), ...(fieldType !== undefined ? { $set: { fieldType } } : {}) }, { new: true }).lean();
