...
export class AssetTransferService {
  constructor(private readonly db: MongooseDatabaseService, @InjectConnection() private readonly connection: Connection) {}
  ...
  async request(auth: AuthContext, assetId: string, dto: { toUserId?: string; toLocationId?: string; toDepartmentId?: string; reason?: string; note?: string }) {
    ...
    const created = await this.db.assetTransfer.create({ tenantId: auth.tenantId, companyId: asset.companyId, assetId, fromUserId: currentUserId, toUserId: dto.toUserId, toLocationId: dto.toLocationId, toDepartmentId: dto.toDepartmentId, requestedByUserId: auth.userId, status: 'PENDING', requestedAt: new Date(), reason: dto.reason, approvalNote: dto.note });
  ...
  async complete(auth: AuthContext, transferId: string, note?: string) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        result = updated;
        await this.db.assetAuditEvent.create([{ tenantId: auth.tenantId, companyId: auth.companyId, assetId: transfer.assetId, fromState: AssetLifecycleState.ASSIGNED, toState: transfer.toUserId ? AssetLifecycleState.ASSIGNED : AssetLifecycleState.IN_STOCK, actorUserId: auth.userId, reason: note?.trim() || 'Asset transfer completed', occurredAt: new Date() }], { session });
      });
