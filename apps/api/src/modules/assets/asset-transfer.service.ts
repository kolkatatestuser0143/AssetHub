...
export class AssetTransferService extends TenantScopedRepository {
  ...
  async complete(auth: AuthContext, transferId: string, note?: string) {
    ...
    try {
      let completed: any;
      let lifecycleFrom: AssetLifecycleState | undefined;
      let lifecycleTo: AssetLifecycleState | undefined;
      await session.withTransaction(async () => {
        ...
        lifecycleFrom = asset.status as AssetLifecycleState;
        lifecycleTo = transfer.toUserId ? AssetLifecycleState.ASSIGNED : AssetLifecycleState.IN_STOCK;
        assertLifecycleTransition(lifecycleFrom, lifecycleTo);
        ...
        const updatedAsset = await this.db.asset.findOneAndUpdate(
          { _id: transfer.assetId, ...this.scope(auth), status: lifecycleFrom },
          { $set: { locationId: transfer.toLocationId, departmentId: transfer.toDepartmentId, status: lifecycleTo, updatedAt: new Date() } },
          { new: true, session },
        ).lean();
      ...
      await this.audit(auth, 'asset.transfer.completed', transferId, {
        ...
        lifecycleFrom: lifecycleFrom ?? null,
        lifecycleTo: lifecycleTo ?? null,
      });
      if (lifecycleFrom && lifecycleTo && lifecycleFrom !== lifecycleTo) {
        await this.db.assetAuditEvent.create({ tenantId: auth.tenantId, companyId: auth.companyId, assetId: transfer.assetId, fromState: lifecycleFrom, toState: lifecycleTo, actorUserId: auth.userId, reason: `Transfer ${transferId}`, occurredAt: new Date() });
      }
