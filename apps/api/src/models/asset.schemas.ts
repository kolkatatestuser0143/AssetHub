...
export class AssetType { @Prop({ required: true, index: true }) companyId!: string; @Prop({ required: true }) name!: string; @Prop({ type: AssetNumberingRuleSchema }) numberingRule?: AssetNumberingRule; }
...
export class AssetAssignment { @Prop({ required: true }) assetId!: string; @Prop({ index: true }) userId?: string; @Prop({ default: Date.now }) assignedAt!: Date; @Prop() returnedAt?: Date; @Prop() notes?: string; @Prop({ enum: AssetCondition }) conditionAtReturn?: string; }
