...
export class AuditInterceptor implements NestInterceptor {
    ...
    return next.handle().pipe(
      tap((value) => {
        void this.audit.record({
          ...
          metadata: { result: 'success', statusCode: res.statusCode, ...this.safeMetadata(value) },
        }).catch(() => undefined);
      }),
      catchError((error) => {
        void this.audit.record({
          ...
          metadata: { result: 'failure', statusCode: error?.status ?? 500, error: String(error?.message ?? 'Request failed').slice(0, 1000) },
        }).catch(() => undefined);
        throw error;
      }),
    );
  ...
  private safeMetadata(value: unknown) {
    ...
    const keys = ['id', 'assetNumber', 'status', 'condition', 'templateName', 'documentId', 'acknowledgementId', 'imported', 'updated', 'deletedCount'];
  ...
  private normalizeRoute(route: string): string {
    return route.replace(/^\/+/, '').replace(/^api\/v1\//, '').replace(/[:/]+/g, '.').replace(/\.\.+/g, '.').replace(/\.+$/g, '').toLowerCase();
  }
  ...
  private inferTargetType(route: string, params: Record<string, unknown>): string | undefined {
    ...
    if (params.assetTypeId || route.includes('/asset-templates') || route.includes('/assets/types')) return 'asset_type';
    if (params.transferId || route.includes('/transfers')) return 'asset_transfer';
    if (params.acknowledgementId || route.includes('/asset-acknowledgements')) return 'asset_acknowledgement';
    if (params.templateId || route.includes('/templates')) return 'template';
    if (params.documentId || route.includes('/documents')) return 'asset_document';
    return undefined;
  }
}
