...
export class AuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const http = context.switchToHttp(); const req = http.getRequest<any>(); const res = http.getResponse<any>(); const method = String(req.method ?? 'GET').toUpperCase(); const route = String(req.route?.path ?? req.originalUrl ?? req.url ?? 'unknown');
    const ignored = method === 'OPTIONS' || route.includes('/api/docs') || route.includes('/health') || route.includes('/audit'); const auth = req.authContext; if (ignored || !auth?.tenantId) return next.handle();
    ...
    const base = { tenantId: auth.tenantId, companyId: auth.companyId, actorUserId: auth.userId, targetType, targetId: targetId ? String(targetId) : undefined, route, method, ipAddress: req.ip, userAgent: String(req.headers?.['user-agent'] ?? '').slice(0, 500) };
    return next.handle().pipe(tap((value) => { void this.audit.record({ ...base, action: `${method.toLowerCase()}.${this.normalizeRoute(route)}`, result: 'success', statusCode: res.statusCode, metadata: this.safeMetadata(value) }).catch(() => undefined); }), catchError((error) => { void this.audit.record({ ...base, action: `${method.toLowerCase()}.${this.normalizeRoute(route)}.failed`, result: 'failure', statusCode: error?.status ?? 500, metadata: { error: String(error?.message ?? 'Request failed').slice(0,1000) } }).catch(() => undefined); throw error; }));
  }
  ...
  private normalizeRoute(route: string): string { return route.replace(/^\/+/, '').replace(/^api\/v1\//, '').replace(/[:/]+/g,'.').replace(/\.\.+/g,'.').replace(/\.+$/g,'').toLowerCase(); }
