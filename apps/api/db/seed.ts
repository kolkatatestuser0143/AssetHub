...
async function main() {
  ...
  try {
    ...
    const existingSystem = await users.findOne({ email: systemEmail });
    if (!existingSystem) {
    ...
    console.log(`Seed complete. Tenant login: ${adminEmail} / ChangeMe123!`);
    console.log(`System login: ${systemEmail} / ${process.env.SYSTEM_ADMIN_PASSWORD ? '<SYSTEM_ADMIN_PASSWORD>' : 'ChangeMe123!'}`);
  } finally {
