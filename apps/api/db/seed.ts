...
async function upsertSeedUser(args: { users: any; email: string; password: string; accountType: UserAccountType; tenantId: string; companyId: string; roleId: string; firstName: string; lastName: string }) {
  const now = new Date();
  const passwordHash = await argon2.hash(args.password, { type: argon2.argon2id });
  await args.users.updateOne(
    { email: args.email },
    { $set: { accountType: args.accountType, tenantId: args.tenantId, companyId: args.companyId, passwordHash, firstName: args.firstName, lastName: args.lastName, forcePasswordReset: true, isActive: true, roleIds: [args.roleId], backupCodesHash: [], updatedAt: now }, $setOnInsert: { _id: new mongoose.Types.ObjectId(), createdAt: now } },
    { upsert: true },
  );
}
...
async function main() {
  ...
  try {
    ...
    const tenantDoc = await tenants.findOne({ slug: 'demo' }); const tenantId = tenantDoc ? tenantDoc._id as mongoose.Types.ObjectId : new mongoose.Types.ObjectId(); if (!tenantDoc) await tenants.insertOne({ _id: tenantId, name: 'Demo Tenant', slug: 'demo', createdAt: now, updatedAt: now });
    const companyDoc = await companies.findOne({ tenantId, code: 'DEMO' }); const companyId = companyDoc ? companyDoc._id as mongoose.Types.ObjectId : new mongoose.Types.ObjectId(); if (!companyDoc) await companies.insertOne({ _id: companyId, tenantId, name: 'Demo Company', code: 'DEMO', createdAt: now, updatedAt: now });
    await roles.updateMany({ name: { $in: Object.keys(SYSTEM_ROLES) }, $or: [{ tenantId }, { tenantId: { $exists: false } }, { tenantId: null }] }, { $set: { tenantId: String(tenantId), updatedAt: now } });
    const roleRefs: Record<string, string> = {};
    for (const [roleName, perms] of Object.entries(SYSTEM_ROLES)) {
      const permRefs = perms.map((key) => { const permissionId = permByKey.get(key); if (!permissionId) throw new Error(`Missing permission key: ${key}`); return { permissionId, permissionKey: key }; });
      const roleId = new mongoose.Types.ObjectId();
      const result = await roles.findOneAndUpdate({ tenantId: String(tenantId), name: roleName }, { $set: { tenantId: String(tenantId), companyId: roleName === 'Platform Admin' ? null : String(companyId), isSystem: true, permissions: permRefs, updatedAt: now }, $setOnInsert: { _id: roleId, createdAt: now } }, { upsert: true, returnDocument: 'after' });
      roleRefs[roleName] = String(result?._id ?? roleId);
    }
    const tenantEmail = process.env.TENANT_ADMIN_EMAIL ?? DEFAULT_TENANT_EMAIL; const tenantPassword = resolveSeedCredential('TENANT_ADMIN_PASSWORD', DEFAULT_TENANT_PASSWORD);
    ...
    const systemEmail = process.env.SYSTEM_ADMIN_EMAIL ?? DEFAULT_SYSTEM_EMAIL; const systemPassword = resolveSeedCredential('SYSTEM_ADMIN_PASSWORD', DEFAULT_SYSTEM_PASSWORD);
    ...
    console.log(`Seed complete. Tenant: ${tenantEmail}`); console.log(`System: ${systemEmail}`);
