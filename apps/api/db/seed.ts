...
const PERMISSIONS = [
  ...
  'company:read', 'company:write', 'user:read', 'user:write',
  'role:read', 'role:write', 'identity_provider:read', 'identity_provider:write',
  'scim:manage', 'integration:read', 'integration:write',
  'billing:read', 'billing:manage', 'audit:read', 'platform:manage_tenants',
];
...
async function main() {
  ...
  try {
    ...
    for (const key of PERMISSIONS) {
      await permissions.updateOne({ key }, { $setOnInsert: { key, _id: new mongoose.Types.ObjectId(), createdAt: now, updatedAt: now } }, { upsert: true });
    }
    ...
    for (const [roleName, perms] of Object.entries(SYSTEM_ROLES)) {
      ...
      if (existing) {
        roleRefs[roleName] = String(existing._id);
        continue;
      }
      const roleId = new mongoose.Types.ObjectId();
      await roles.insertOne({ _id: roleId, tenantId, companyId: roleName === 'Platform Admin' ? null : companyId, name: roleName, isSystem: true, permissions: permRefs, createdAt: now, updatedAt: now });
    ...
    if (!existingAdmin) {
      ...
      await users.insertOne({ _id: new mongoose.Types.ObjectId(), tenantId, companyId, email: adminEmail, passwordHash, firstName: 'Demo', lastName: 'Admin', forcePasswordReset: true, isActive: true, roleIds: [roleRefs['Tenant Admin']], backupCodesHash: [], createdAt: now, updatedAt: now });
    }
