...
const TENANT_EMAIL = (process.env.TENANT_ADMIN_EMAIL ?? 'admin@demo.local').trim().toLowerCase();
const TENANT_PASSWORD = process.env.TENANT_ADMIN_PASSWORD ?? 'ChangeMe1234567!';
const SYSTEM_EMAIL = (process.env.SYSTEM_ADMIN_EMAIL ?? 'admin@assethub.local').trim().toLowerCase();
...
async function resetTenantAccount(db: any, now: Date) {
  ...
  const saved = await users.findOne({ _id: user._id }, { projection: { passwordHash: 1, email: 1, adminLevel: 1, isActive: 1 } });
  if (!saved?.passwordHash || !(await argon2.verify(saved.passwordHash, TENANT_PASSWORD))) throw new Error('Tenant demo credential verification failed after reset.');
...
async function resetSystemAccount(db: any, now: Date) {
  ...
  const saved = await users.findOne({ _id: user._id }, { projection: { passwordHash: 1, email: 1, accountType: 1, isActive: 1 } });
  if (!saved?.passwordHash || !(await argon2.verify(saved.passwordHash, SYSTEM_PASSWORD))) throw new Error('System demo credential verification failed after reset.');
  await sessions.updateMany({ userId: String(user._id), revokedAt: { $exists: false } }, { $set: { revokedAt: now, revokedReason: 'demo_credentials_reset' } });
}
...
async function main() {
  ...
  try {
    const db = connection.db; if (!db) throw new Error('Mongo connection failed: native db handle is undefined');
    const now = new Date();
