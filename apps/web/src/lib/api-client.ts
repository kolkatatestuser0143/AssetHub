...
async function refreshSession() {
  ...
  refreshing = fetch(`${API_BASE}/auth/refresh`, {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Auth-Scope': 'tenant' }, credentials: 'include',
  }).then((res) => { if (!res.ok) throw new Error('Session expired'); }).finally(() => { refreshing = null; });
  return refreshing;
}
...
export async function downloadFile(path: string, retry = true, options: RequestInit = {}) {
  const res = await fetch(`${API_BASE}${path}`, { ...options, headers: { ...(options.body ? { 'Content-Type': 'application/json' } : {}), ...(options.headers ?? {}) }, credentials: 'include' });
  if (res.status === 401 && retry && path !== '/auth/refresh') { await refreshSession(); return downloadFile(path, false, options); }
  if (!res.ok) { const body = await res.json().catch(() => ({})); throw new Error(body.message ?? `Request failed: ${res.status}`); }
...
export async function login(email: string, password: string) { return apiFetch('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) }); }
...
export async function logout() {
  ...
  try { await fetch(`${API_BASE}/auth/logout`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Auth-Scope': 'tenant' }, credentials: 'include' }); } catch { /* Server logout may be unreachable. */ }
  sessionStorage.removeItem('itam_refresh_token'); sessionStorage.removeItem('itam_access_token');
}
