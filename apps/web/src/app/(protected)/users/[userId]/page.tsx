...
import { ArrowLeft, Check, ChevronDown, RefreshCw, ShieldCheck, UserCheck, UserX, Users } from 'lucide-react';
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 space-y-3">{(user.roleIds?.length ?? 0) === 0 ? <div className="rounded-xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">No roles assigned.</div> : user.roleIds?.map((roleId) => { const role = roleById(roleId); return <div key={roleId} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-4"><div><p className="font-semibold text-slate-900">{role?.name ?? 'Role'}</p><p className="mt-1 font-mono text-xs text-slate-400">{roleId}</p></div>{role?.isSystem && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">System role</span>}</div>{role?.permissions?.length ? <div className="mt-4 flex flex-wrap gap-2">{role.permissions.map((item) => { const key = permissionKey(item); return key ? <span key={`${roleId}-${key}`} className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700"><Check size={12}/>{key}</span> : null; })}</div> : null}</div>; })}</div>
      </section>
