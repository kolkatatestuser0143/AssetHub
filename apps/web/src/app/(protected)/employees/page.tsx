...
export default function EmployeesPage(){
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 page-section-enter">
    ...
    <section className="panel overflow-hidden"><div className="border-b border-slate-100 p-4 sm:p-5"><label className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 shadow-sm"><Search size={16} className="text-slate-400"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search name, email, employee ID or company…" className="w-full bg-transparent py-2 text-sm outline-none"/></label></div>
      {loading?<div className="p-8 text-sm text-slate-500">Loading employees…</div>:filtered.length===0?<div className="p-12 text-center"><UserRound size={32} className="mx-auto text-slate-300"/><p className="mt-3 font-semibold text-slate-800">No employees found in this scope</p><p className="mt-1 text-sm text-slate-500">Try another company or clear the search filter.</p></div>:<div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Employee</th><th className="px-5 py-3">Company</th><th className="px-5 py-3">Job title</th><th className="px-5 py-3">Access level</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((u)=><tr key={u.id} className="hover:bg-[var(--theme-primary-soft)]/35"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-500"><UserRound size={17}/></span><div><Link href={`/users/${u.id}`} className="font-semibold text-slate-900 hover:text-[var(--theme-link)]">{u.firstName} {u.lastName}</Link><div className="mt-0.5 text-xs text-slate-500">{u.email}</div><div className="mt-0.5 font-mono text-[11px] text-slate-400">{u.employeeId||'Employee ID not set'}</div></div></div></td><td className="px-5 py-4"><div className="font-medium text-slate-700">{companyName(u.companyId)}</div><div className="text-[11px] text-slate-400">{u.companyId||'No company assigned'}</div></td><td className="px-5 py-4 text-slate-600">{u.jobTitle||'—'}</td><td className="px-5 py-4"><span className={u.adminLevel==='COMPANY_ADMIN'?'badge badge-brand':'badge badge-neutral'}>{LEVEL_LABEL[u.adminLevel??'EMPLOYEE']}</span></td><td className="px-5 py-4"><span className={u.isActive?'badge badge-success':'badge badge-neutral'}>{u.isActive?'Active':'Inactive'}</span></td><td className="px-5 py-4 text-right">{u.adminLevel==='TENANT_ADMIN'?<span className="text-xs font-semibold text-slate-400">Tenant Admins</span>:<button onClick={()=>void promote(u)} disabled={!u.isActive||busy===u.id} className="btn-secondary btn-sm ui-interactive"><ShieldCheck size={14}/>{busy===u.id?'Promoting…':'Promote to Tenant Admin'}<ArrowUpRight size={13}/></button>}</td></tr>)}</tbody></table></div>}
    </section>
  </div>
}
