...
import { Boxes, CheckSquare, ChevronLeft, ChevronRight, Download, Plus, RefreshCw, Search, SlidersHorizontal, Upload, X } from 'lucide-react';
...
export default function AssetsPage() {
  ...
  const [newTypeName, setNewTypeName] = useState(''), [newTypePrefix, setNewTypePrefix] = useState(''), [query, setQuery] = useState(''), [statusFilter, setStatusFilter] = useState('ALL'), [assetTypeFilter, setAssetTypeFilter] = useState('');
  const [page, setPage] = useState(1), [pageSize] = useState(25), [pagination, setPagination] = useState<PageResponse['pagination']>({ page: 1, pageSize: 25, total: 0, totalPages: 1 });
  ...
  async function loadTypes() {
    try { const data = await apiFetch('/assets/types'); const next = Array.isArray(data) ? data : []; setAssetTypes(next); if (next.length > 0 && !selectedTypeId) setSelectedTypeId(next[0].id); }
  ...
  }, [page, pageSize, query, statusFilter, assetTypeFilter]);
  ...
  function exportSelected() {
    const rows = [['assetId','assetNumber','status','assetType'], ...assets.filter((asset) => selected.includes(asset.id)).map((asset) => [asset.id, asset.assetNumber, asset.status, asset.assetType?.name ?? ''])];
  ...
  return <div className="mx-auto max-w-[1400px] space-y-6">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--theme-link)]">Inventory</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">Assets</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">Manage the tenant inventory lifecycle, identifiers, and operational state from one workspace.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => void loadAssets(page)} disabled={busy} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-60"><RefreshCw size={16} className={busy ? 'animate-spin' : ''}/>Refresh</button><Link href="/assets/transfers" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Transfers</Link><Link href="/assets/reports" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50"><Download size={16}/>Reports</Link><button onClick={exportExcel} disabled={exporting} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-60"><Download size={16}/>{exporting ? 'Generating…' : 'Quick Excel'}</button><Link href="/assets/import" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50"><Upload size={16}/>Import</Link>{assetTypes.length > 0 && <button onClick={createAsset} className="inline-flex items-center gap-2 rounded-xl bg-[var(--theme-primary)] px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[var(--theme-primary-hover)]"><Plus size={16}/>New asset</button>}</div></div>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-900"><Boxes size={17} className="text-[var(--theme-link)]"/>Asset type setup</div><form onSubmit={createType} className="grid gap-3 md:grid-cols-[1fr_180px_auto]"><input required value={newTypeName} onChange={(e) => setNewTypeName(e.target.value)} placeholder="Type name, e.g. Laptop" className="field h-11"/><input required value={newTypePrefix} onChange={(e) => setNewTypePrefix(e.target.value)} placeholder="Prefix, e.g. LAP" className="field h-11 uppercase"/><button className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white hover:bg-slate-800"><Plus size={16}/>Create type</button></form>{assetTypes.length > 0 && <div className="mt-4 flex flex-wrap items-center gap-3"><label className="text-sm font-medium text-slate-700">Create using type</label><select value={selectedTypeId} onChange={(e) => setSelectedTypeId(e.target.value)} className="field h-10 w-auto min-w-48">{assetTypes.map((type) => <option key={type.id} value={type.id}>{type.name}{type.prefix ? ` · ${type.prefix}` : ''}</option>)}</select></div>}</section>
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-100 p-4 md:flex-row md:items-center md:justify-between"><div className="relative w-full max-w-md"><Search size={16} className="absolute left-3 top-2.5 text-slate-400"/><input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); setSelected([]); }} placeholder="Search asset number, type, status" className="field h-10 pl-9"/></div><div className="flex flex-wrap items-center gap-2"><SlidersHorizontal size={16} className="text-slate-400"/><select value={statusFilter} onChange={(e) => changeStatus(e.target.value)} className="field h-10 w-auto"><option value="ALL">All statuses</option>{STATES.map((state) => <option key={state} value={state}>{state}</option>)}</select><select value={assetTypeFilter} onChange={(e) => changeType(e.target.value)} className="field h-10 w-auto"><option value="">All asset types</option>{assetTypes.map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select><select value={density} onChange={(e) => setDensity(e.target.value as 'comfortable' | 'compact')} className="field h-10 w-auto"><option value="comfortable">Comfortable</option><option value="compact">Compact</option></select><span className="text-xs text-slate-500">{selected.length ? `${selected.length} selected · ` : ''}{pagination.total} assets</span></div></div>
      {selected.length > 0 && <div className="sticky top-16 z-20 flex flex-col gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur sm:flex-row sm:items-center"><div className="flex items-center gap-2 text-sm font-semibold text-slate-900"><CheckSquare size={16} className="text-[var(--theme-link)]"/>{selected.length} selected on this page</div><div className="flex flex-1 flex-wrap gap-2"><select value={bulkState} onChange={(e) => setBulkState(e.target.value)} className="field h-9 w-auto"><option value="">Change lifecycle…</option>{STATES.map((state) => <option key={state} value={state}>{state}</option>)}</select><button disabled={!bulkState || bulkBusy} onClick={() => void bulkTransition()} className="inline-flex h-9 items-center rounded-lg bg-[var(--theme-primary)] px-3 text-xs font-semibold text-white disabled:opacity-50">{bulkBusy ? 'Updating…' : 'Apply'}</button><button onClick={exportSelected} className="inline-flex h-9 items-center gap-1 rounded-lg border px-3 text-xs font-semibold"><Download size={13}/>Export selected</button></div><button onClick={() => setSelected([])} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-900"><X size={14}/>Clear</button></div>}
      {busy ? <div className="space-y-3 p-5">{[1,2,3,4,5].map((n) => <div key={n} className="h-12 animate-pulse rounded-xl bg-slate-100"/>)}</div> : assets.length === 0 ? <div className="p-14 text-center"><Boxes className="mx-auto text-slate-300" size={38}/><p className="mt-4 font-semibold text-slate-800">No assets found</p><p className="mt-1 text-sm text-slate-500">Try changing your search or filters.</p></div> : <div className="max-h-[70vh] overflow-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="w-12 px-4 py-3"><input type="checkbox" aria-label="Select visible assets" checked={allVisibleSelected} onChange={toggleAll}/></th><th className="px-5 py-3">Asset</th><th className="px-5 py-3">Type</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Lifecycle</th><th className="px-5 py-3"/></tr></thead><tbody className="divide-y divide-slate-100">{assets.map((asset) => <tr key={asset.id} className={`transition hover:bg-[var(--theme-primary-soft)]/60 ${selected.includes(asset.id) ? 'bg-[var(--theme-primary-soft)]' : ''}`}><td className="px-4 py-3"><input type="checkbox" aria-label={`Select ${asset.assetNumber}`} checked={selected.includes(asset.id)} onChange={() => toggleOne(asset.id)}/></td><td className={rowPad}><Link href={`/assets/${asset.id}`} className="font-semibold text-slate-900 hover:text-[var(--theme-link)]">{asset.assetNumber}</Link><div className="font-mono text-[11px] text-slate-400">{asset.id}</div></td><td className={rowPad + ' text-slate-700'}>{asset.assetType?.name ?? '—'}</td><td className={rowPad}><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{asset.status}</span></td><td className={rowPad}><select defaultValue="" onChange={(e) => e.target.value && transition(asset.id, e.target.value)} className="field h-9 w-auto min-w-32 text-xs"><option value="">Change state…</option>{STATES.filter((state) => state !== asset.status).map((state) => <option key={state} value={state}>{state}</option>)}</select></td><td className={rowPad + ' text-right'}><Link href={`/assets/${asset.id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--theme-link)]">Details<ChevronRight size={14}/></Link></td></tr>)}</tbody></table></div>}
