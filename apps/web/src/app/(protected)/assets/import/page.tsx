...
import { ArrowLeft, CheckCircle2, FileUp, Upload, XCircle } from 'lucide-react';
...
export default function AssetImportPage() {
  ...
  async function onFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 5 * 1024 * 1024) { setError('CSV file cannot exceed 5 MB.'); return; }
    setError(''); setMessage(''); setPreview(null); setFileName(file.name); setCsv(await file.text());
  }
  async function previewImport() { setBusy(true); setError(''); setMessage(''); setPreview(null); try { setPreview(await apiFetch('/assets/import/preview', { method: 'POST', body: JSON.stringify({ csv }) })); } catch (e: any) { setError(e?.message ?? 'Unable to validate CSV.'); } finally { setBusy(false); } }
  async function commitImport() { if (!preview) return; setBusy(true); setError(''); setMessage(''); try { const result = await apiFetch('/assets/import', { method: 'POST', body: JSON.stringify({ csv }) }); setMessage(`Imported ${result.imported ?? 0} assets successfully.`); setCsv(''); setFileName(''); setPreview(null); } catch (e: any) { setError(e?.message ?? 'Unable to import assets.'); } finally { setBusy(false); } }
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"><FileUp size={16}/>Choose CSV<input type="file" accept=".csv,text/csv" onChange={onFileChange} className="hidden"/></label>{fileName && <span className="text-sm text-slate-500">{fileName}</span>}</div>
      <p className="mt-3 text-xs leading-5 text-slate-500">Required: <code>assetTypeId</code>. Optional: <code>locationId</code>, <code>departmentId</code>, <code>vendorId</code>, <code>fieldsJson</code>. Maximum 5,000 rows and 5 MB per import.</p>
      <textarea value={csv} onChange={(e) => { setCsv(e.target.value); setFileName(''); setPreview(null); }} rows={16} spellCheck={false} placeholder={'assetTypeId,locationId,departmentId,vendorId,fieldsJson\n665...,665...,665...,665...,"{\"serialNumber\":\"ABC-001\"}"'} className="mt-4 w-full rounded-xl border border-slate-200 p-4 font-mono text-xs outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
