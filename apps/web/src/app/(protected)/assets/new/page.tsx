...
type SiteType = 'plant' | 'branch_office' | 'head_office';
...
type FormState = { assetTypeId: string; serialNumber: string; model: string; vendorId: string; condition: string; siteId: string; locationId: string; departmentId: string };
const CONDITIONS = ['GOOD', 'FAIR', 'DAMAGED', 'NEEDS_INSPECTION'];
const SITE_LABEL: Record<SiteType, string> = { plant: 'Plant', branch_office: 'Branch Office', head_office: 'Head Office' };
...
export default function NewAssetPage() {
  ...
  const [form, setForm] = useState<FormState>({ assetTypeId: '', serialNumber: '', model: '', vendorId: '', condition: 'GOOD', siteId: '', locationId: '', departmentId: '' });
  ...
  async function load() {
    ...
    try {
      const [typeData, vendorData, hierarchyData] = await Promise.all([apiFetch('/assets/types'), apiFetch('/assets/vendors'), apiFetch('/companies/hierarchy')]);
      const nextTypes = Array.isArray(typeData) ? typeData : [];
      setTypes(nextTypes); setVendors(Array.isArray(vendorData) ? vendorData : []); setHierarchy(Array.isArray(hierarchyData) ? hierarchyData : []);
      setForm((v) => ({ ...v, assetTypeId: v.assetTypeId || nextTypes[0]?.id || '' }));
  ...
  const currentCompany = useMemo(() => hierarchy.find((c) => c.id === tenantProfile?.company?.id) || hierarchy[0], [hierarchy, tenantProfile?.company?.id]);
  const selectedSite = currentCompany?.sites?.find((x) => x.id === form.siteId);
  const selectedLocation = selectedSite?.locations?.find((x) => x.id === form.locationId);
  const clearOrganization = () => setForm((v) => ({ ...v, siteId: '', locationId: '', departmentId: '' }));

  async function submit(e: React.FormEvent) {
    e.preventDefault(); if (!form.assetTypeId) { setError('Asset type is required.'); return; }
    setSaving(true); setError(null);
    try {
      await apiFetch('/assets', { method: 'POST', body: JSON.stringify({ assetTypeId: form.assetTypeId, locationId: form.locationId || undefined, departmentId: form.departmentId || undefined, vendorId: form.vendorId || undefined, condition: form.condition, serialNumber: form.serialNumber.trim() || undefined, model: form.model.trim() || undefined }) });
      router.push('/assets');
    } catch (e: any) { setError(e?.message || 'Unable to create asset.'); }
    finally { setSaving(false); }
  }

  const companyName = tenantProfile?.name || currentCompany?.name || 'Tenant account';

  return <div className="mx-auto max-w-4xl space-y-6"><div className="flex items-center gap-3"><Link href="/assets" className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"><ArrowLeft size={18}/></Link><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--theme-link)]">Inventory</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">New asset</h1><p className="mt-1 text-sm text-slate-500">Create an asset record for {companyName}. Organization details are optional.</p></div></div>
    ...
    <form onSubmit={submit} className="space-y-5"><section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="font-semibold text-slate-950">Asset information</h2><p className="mt-1 text-sm text-slate-500">All fields except asset type are optional.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="space-y-1.5 text-sm sm:col-span-2"><span className="font-medium text-slate-700">Asset type <span className="text-red-500">*</span></span><select required disabled={loading} value={form.assetTypeId} onChange={(e) => set('assetTypeId', e.target.value)} className="field h-11 w-full"><option value="">Select asset type</option>{types.map((t) => <option key={t.id} value={t.id}>{t.name}{t.prefix ? ` · ${t.prefix}` : ''}</option>)}</select></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Serial number</span><input value={form.serialNumber} onChange={(e) => set('serialNumber', e.target.value)} placeholder="e.g. ABC123456" className="field h-11 w-full"/></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Model</span><input value={form.model} onChange={(e) => set('model', e.target.value)} placeholder="e.g. Latitude 5440" className="field h-11 w-full"/></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Vendor</span><select value={form.vendorId} onChange={(e) => set('vendorId', e.target.value)} className="field h-11 w-full"><option value="">Select vendor</option>{vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Condition</span><select value={form.condition} onChange={(e) => set('condition', e.target.value)} className="field h-11 w-full">{CONDITIONS.map((c) => <option key={c} value={c}>{c.replaceAll('_', ' ')}</option>)}</select></label></div></section>
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><button type="button" onClick={() => setShowOrganization((v) => !v)} className="flex w-full items-center justify-between px-6 py-5 text-left"><div><h2 className="font-semibold text-slate-950">Organization <span className="font-normal text-slate-400">(optional)</span></h2><p className="mt-1 text-sm text-slate-500">Optionally assign this asset to a site, location or department.</p></div><ChevronDown size={18} className={showOrganization ? 'rotate-180' : ''}/></button>{showOrganization && <div className="border-t border-slate-100 p-6"><div className="mb-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Company: <span className="font-semibold text-slate-800">{currentCompany?.name || 'Current company'}</span> <span className="text-slate-400">({currentCompany?.code || '—'})</span> <span className="ml-2 text-xs text-slate-400">selected automatically</span></div>{currentCompany?.sites?.length ? <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1.5 text-sm sm:col-span-2"><span className="font-medium text-slate-700">Plant / Branch / Head Office</span><select value={form.siteId} onChange={(e) => { const id = e.target.value; set('siteId', id); set('locationId', ''); set('departmentId', ''); }} className="field h-11 w-full"><option value="">Not assigned</option>{currentCompany.sites.map((x) => <option key={x.id} value={x.id}>{x.name} · {SITE_LABEL[x.type ?? 'plant']}</option>)}</select></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Location</span><select value={form.locationId} disabled={!selectedSite} onChange={(e) => { set('locationId', e.target.value); set('departmentId', ''); }} className="field h-11 w-full"><option value="">Not assigned</option>{selectedSite?.locations.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label className="space-y-1.5 text-sm"><span className="font-medium text-slate-700">Department</span><select value={form.departmentId} disabled={!selectedLocation} onChange={(e) => set('departmentId', e.target.value)} className="field h-11 w-full"><option value="">Not assigned</option>{selectedLocation?.departments.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label></div> : <div className="rounded-xl border border-dashed border-slate-200 px-4 py-5 text-sm text-slate-500">No organization structure has been created for this company. You can create the asset normally without it.</div>}{(form.siteId || form.locationId || form.departmentId) && <button type="button" onClick={clearOrganization} className="mt-4 text-xs font-semibold text-[var(--theme-link)]">Clear organization assignment</button>}</div>}</section>
