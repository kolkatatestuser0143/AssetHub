...
export default function VendorsPage() {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      if (editing) {
        await apiFetch(`/assets/vendors/${editing.id}`, { method: 'PATCH', body: JSON.stringify({ name: name.trim(), contact: contact.trim() || undefined }) });
      } else {
        await apiFetch('/assets/vendors', { method: 'POST', body: JSON.stringify({ name: name.trim(), contact: contact.trim() || undefined }) });
      }
      cancelEdit();
      await load();
    } catch (err: any) {
  ...
  return <div className="mx-auto max-w-[1200px] space-y-6">
    ...
    {error && <ErrorState title="Unable to load vendors" message={error} onRetry={() => void load()} />}
    <section className="panel p-5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Plus size={17} className="text-[var(--theme-link)]"/><h2 className="font-semibold text-slate-950">{editing ? 'Edit vendor' : 'Add vendor'}</h2></div>{editing && <Button type="button" variant="ghost" size="sm" onClick={cancelEdit} icon={<X size={14}/>}>Cancel</Button>}</div>
      <form onSubmit={save} className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_auto]"><FormField label="Vendor name" required value={name} onChange={(e)=>setName(e.target.value)} placeholder="Vendor name"/><FormField label="Contact" value={contact} onChange={(e)=>setContact(e.target.value)} placeholder="Contact / email / phone"/><div className="flex items-end"><Button className="h-11 w-full" loading={saving} icon={<Plus size={16}/>}>{editing ? 'Save changes' : 'Add vendor'}</Button></div></form>
    </section>
