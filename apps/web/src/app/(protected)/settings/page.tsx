...
import { Bell, Lock, ShieldCheck, Settings2, FileKey2, ChevronRight } from 'lucide-react';
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div className="flex gap-3">
            <FileKey2 size={19} className="mt-0.5 text-blue-600" />
            <div>
              <h2 className="font-semibold text-slate-950">License & subscription</h2>
              <p className="mt-1 text-sm text-slate-500">View your tenant plan, license status, limits, features, and current usage.</p>
            </div>
          </div>
          <Link href="/settings/license" className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">View license<ChevronRight size={15}/></Link>
        </div>
      </section>
    </div>
  );
