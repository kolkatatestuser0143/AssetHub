...
const NAV = [
  ['/dashboard','Dashboard',LayoutDashboard], ['/assets','Assets',Laptop], ['/asset-types','Asset types',Boxes],
  ['/companies','Companies & structure',Building2], ['/assignments','Assignments',ListChecks], ['/vendors','Vendors',Truck],
  ['/custom-fields','Custom fields',SlidersHorizontal], ['/roles','Roles & permissions',ShieldCheck], ['/identity','Identity & SSO',FileKey2],
  ['/audit','Audit log',ClipboardList], ['/reports','Reports',BarChart3], ['/users','Users',Users],
] as const;

export default function ProtectedLayout({children}:{children:React.ReactNode}) {
  const {status,logout,themePreset}=useAuth(); const pathname=usePathname(); const [open,setOpen]=useState(false);
