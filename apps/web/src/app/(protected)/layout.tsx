...
type NavItem = readonly [string, string, typeof LayoutDashboard, FeatureKey?];
...
const NAV: readonly NavItem[] = [
  ...
  ['/custom-fields','Custom fields',SlidersHorizontal,FEATURE_KEYS.customFields],
  ['/roles','Roles & permissions',ShieldCheck,FEATURE_KEYS.customRoles],
  ['/identity','Identity & SSO',FileKey2,FEATURE_KEYS.sso],
  ['/audit','Audit log',ClipboardList,FEATURE_KEYS.audit],
  ['/reports','Reports',BarChart3,FEATURE_KEYS.advancedReports],
  ['/users','Users',Users],
];
...
function getActiveHref(pathname: string, items: readonly NavItem[]) {
  return items.map(([href]) => href).filter((href) => isRouteMatch(pathname, href)).sort((a, b) => b.length - a.length)[0] ?? null;
}
...
function ProtectedContent({ children }: { children: React.ReactNode }) {
  ...
  const activeHref = useMemo(() => getActiveHref(pathname, visibleNav), [pathname, visibleNav]);
  ...
  if (requiredFeature && !hasFeature(requiredFeature)) return <div className="grid min-h-screen place-items-center bg-slate-50 text-sm text-slate-500">This feature is not included in your current license.</div>;

  const sidebar = <aside className="flex h-full flex-col text-slate-300" style={{ background: 'var(--theme-sidebar)', color: 'var(--theme-sidebar-text)' }}>
    <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5"><div className="grid h-9 w-9 place-items-center rounded-xl font-bold text-white" style={{ background: 'var(--theme-primary)' }}>A</div><div><div className="font-semibold text-white">AssetHub</div><div className="text-[10px] uppercase tracking-widest opacity-60">ITAM Platform</div></div></div>
    <nav className="flex-1 overflow-y-auto px-3 py-5"><p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-widest opacity-60">Workspace</p>{visibleNav.map(([href,label,Icon])=>{const active=activeHref===href;return <Link key={href} href={href} prefetch onClick={()=>setOpen(false)} className={`mb-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active?'text-white shadow-sm':''}`} style={active?{background:'var(--theme-sidebar-active)'}:undefined}><Icon size={18}/>{label}</Link>})}</nav>
  ...
  return <div className="min-h-screen bg-slate-50"><div className="fixed inset-y-0 left-0 hidden w-64 lg:block">{sidebar}</div>{open&&<div className="fixed inset-0 z-50 bg-slate-950/60 lg:hidden"><div className="h-full w-72">{sidebar}</div></div>}<div className="lg:pl-64"><header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6"><button className="rounded-lg p-2 lg:hidden" onClick={()=>setOpen(true)} aria-label="Open navigation">{open?<X size={20}/>:<Menu size={20}/>}</button><div className="ml-auto flex items-center gap-2"><button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Notifications"><Bell size={19}/></button><div className="hidden sm:flex items-center gap-2 rounded-xl border border-slate-200 px-2 py-1.5"><kbd className="rounded-md border bg-slate-50 px-1.5 py-1 text-[10px] text-slate-400">⌘K</kbd><span className="text-[11px] text-slate-500">Search</span></div><div className="flex items-center gap-2 rounded-xl border border-slate-200 px-2 py-1.5"><span className="grid h-8 w-8 items-center justify-center rounded-lg text-xs font-bold" style={{background:'var(--theme-primary-soft)',color:'var(--theme-link)'}}>A</span><div className="hidden md:block"><div className="text-xs font-semibold">Tenant account</div><div className="text-[11px] text-slate-500">{themePreset.replace(/_/g,' ')}</div></div></div></div></header><main className="p-4 sm:p-6 lg:p-8"><PageTransition>{children}</PageTransition></main></div><CommandPalette/></div>;
}
