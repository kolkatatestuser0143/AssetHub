...
export default function SystemLayout({ children }: { children: React.ReactNode }) {
  ...
  const [authenticated, setAuthenticated] = useState(isLoginPage ? false : false);
  ...
  const sidebar = (
    <aside className="fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 text-white shadow-2xl">
      <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
        <Link href="/system" className="flex items-center gap-3" onClick={() => {}}>
        ...
        <button className="rounded-lg p-2 text-slate-400 hover:bg-white/10 hover:text-white lg:hidden" onClick={() => {}}><X className="h-5 w-5" /></button>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        ...
        <div className="space-y-1">
          {navigation.map((item) => {
            ...
            return <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white'}`}><Icon className="h-4 w-4" />{item.label}</Link>;
          })}
        </div>
      </nav>
      ...
      <div className="min-h-screen lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
          <div className="flex h-16 items-center gap-4 px-4 sm:px-6">
            <button className="rounded-xl border border-slate-200 p-2 text-slate-600 lg:hidden" onClick={() => {}} aria-label="Open navigation"><Menu className="h-5 w-5" /></button>
