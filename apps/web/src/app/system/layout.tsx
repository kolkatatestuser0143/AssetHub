...
import { Activity, BarChart3, Building2, CreditCard, FileSearch, LayoutDashboard, LogOut, Menu, Settings, ShieldCheck, Users, X, type LucideIcon } from 'lucide-react';
...
export default function SystemLayout({ children }: { children: React.ReactNode }) {
  ...
  if (checkingAuth || !authenticated) return <div className="grid min-h-screen place-items-center bg-slate-950 text-sm text-slate-400" style={SYSTEM_THEME}>Loading system console…</div>;
  ...
  const sidebar = (
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 text-white shadow-2xl transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex h-16 items-center justify-between border-b border-white/10 px-5"><Link href="/system" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}><span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-600 shadow-lg shadow-blue-600/20"><ShieldCheck className="h-5 w-5" /></span><div><p className="font-semibold tracking-tight">AssetHub</p><p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Platform</p></div></Link><button className="rounded-lg p-2 text-slate-400 hover:bg-white/10 hover:text-white lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation"><X className="h-5 w-5" /></button></div>
