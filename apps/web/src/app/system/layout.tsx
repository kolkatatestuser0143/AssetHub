...
import { NetworkStatus, ProductivityHub } from '../../components/productivity-hub';
...
export default function SystemLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const router = useRouter(); const isLoginPage = pathname === LOGIN_PATH; const [checkingAuth, setCheckingAuth] = useState(!isLoginPage); const [authenticated, setAuthenticated] = useState(false); const [mobileOpen, setMobileOpen] = useState(false);
  ...
  if (isLoginPage) return <div data-system-theme data-system-console className="min-h-screen" style={SYSTEM_THEME}>{children}</div>;
  if (checkingAuth || !authenticated) return <div data-system-theme data-system-console className="grid min-h-screen place-items-center bg-slate-950 text-sm text-slate-400" style={SYSTEM_THEME}>Loading system console…</div>;
  ...
  return <div data-system-theme data-system-console className="min-h-screen bg-slate-950 text-slate-950" style={SYSTEM_THEME}><ConnectionBanner/><Sidebar groups={sidebarGroups} variant="system" mobileOpen={mobileOpen} onMobileClose={() => setMobileOpen(false)} brand={brand} context={context} footer={footer} ariaLabel="System navigation" /><div className="min-h-screen lg:pl-64"><header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="flex min-h-16 items-center gap-4 px-4 py-2 sm:px-6"><button className="rounded-xl border border-slate-200 p-2 text-slate-600 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation">☰</button><div className="min-w-0 flex-1"><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">AssetHub Platform</p><h1 className="truncate text-lg font-bold tracking-tight text-slate-950">{title}</h1></div><NetworkStatus/><ProductivityHub/><div className="hidden items-center gap-3 sm:flex"><div className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">System session active</div><button onClick={logout} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Sign out</button></div></div></header><main className="min-h-[calc(100vh-4rem)]"><div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">{children}</div></main></div><SystemModalBridge/></div>;
}
