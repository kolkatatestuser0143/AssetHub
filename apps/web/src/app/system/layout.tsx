...
export default function SystemLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoginPage) {
    return (
      <div data-system-theme className="min-h-screen" style={SYSTEM_THEME}>
        {children}
      </div>
    );
  }
  ...
  if (checkingAuth || !authenticated) {
    return (
      <div
        data-system-theme
        className="grid min-h-screen place-items-center bg-slate-950 text-sm text-slate-400"
        style={SYSTEM_THEME}
      >
  ...
  const sidebar = (
    <aside
      className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-slate-950 text-white shadow-2xl transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
    >
      <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
        ...
        >
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-blue-600 shadow-lg shadow-blue-600/20">
            <ShieldCheck className="h-5 w-5" />
          </span>
          <div>
            <p className="font-semibold tracking-tight">AssetHub</p>
            <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Platform</p>
          </div>
        </Link>
        <button
      ...
      <div className="border-b border-white/10 px-5 py-4">
        <div className="rounded-xl border border-blue-400/20 bg-blue-500/10 p-3">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-300">System Administrator</p>
          <p className="mt-1 text-sm font-medium text-white">Platform Control Center</p>
        </div>
      </div>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-300 hover:bg-white/5 hover:text-white'}`}
              >
      ...
      <div className="border-t border-white/10 p-3">
        <button
          onClick={logout}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-red-500/10 hover:text-red-200"
        >
  ...
  return (
    <div data-system-theme className="min-h-screen bg-slate-950 text-slate-950" style={SYSTEM_THEME}>
      {mobileOpen && (
        <button
          aria-label="Close navigation"
