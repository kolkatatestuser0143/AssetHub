...
export default function OrganizationLevelPage({ level }: { level: Level }) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      {editor ? (
        <div className="fixed inset-0 z-[70] flex items-stretch justify-end bg-slate-950/45 p-0 backdrop-blur-sm sm:p-3">
          <div className="flex h-full w-full max-w-xl flex-col overflow-hidden bg-white shadow-2xl sm:rounded-2xl">
            <div className="flex flex-shrink-0 items-start justify-between border-b border-slate-100 px-6 py-5">
              <div>
                ...
                <h2 className="mt-1 text-xl font-bold text-slate-950">{editor.mode === 'create' ? `Add ${noun}` : `Edit ${noun}`}</h2>
              </div>
              <button type="button" onClick={() => !busy && setEditor(null)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Close"><X size={18}/></button>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              <div className="space-y-4">
                {level === 'sites' ? <>
                  <FormSelect label="Company" id="org-company" value={editor.companyId ?? ''} onChange={(e) => setEditor((s) => s ? { ...s, companyId: e.target.value } : s)} required>
                    <option value="">Select company</option>{companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </FormSelect>
                  <FormSelect label="Site type" id="org-site-type" value={editor.type ?? 'plant'} onChange={(e) => setEditor((s) => s ? { ...s, type: e.target.value } : s)} required>
                    {SITE_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </FormSelect>
                </> : level === 'locations' ? (
                  formSelect('Site', 'org-site', editor.siteId ?? '', (value) => setEditor((s) => s ? { ...s, siteId: value } : s), companies.flatMap((c) => (c.sites ?? []).map((site) => ({ value: site.id, label: `${site.name} — ${c.name}` }))), 'Select site')
                ) : (
                  formSelect('Location', 'org-location', editor.locationId ?? '', (value) => setEditor((s) => s ? { ...s, locationId: value } : s), companies.flatMap((c) => (c.sites ?? []).flatMap((site) => (site.locations ?? []).map((location) => ({ value: location.id, label: `${location.name} — ${site.name}` })))), 'Select location')
                )}
                <FormField label="Name" id="org-name" value={editor.name} onChange={(e) => setEditor((s) => s ? { ...s, name: e.target.value } : s)} placeholder={`Enter ${noun.toLowerCase()} name`} required />
              </div>
            </div>
            <div className="flex flex-shrink-0 justify-end gap-2 border-t border-slate-100 bg-white px-6 py-4">
              ...
              <Button onClick={() => void save()} loading={busy}>{editor.mode === 'create' ? 'Create' : 'Save changes'}</Button>
            </div>
          </div>
        </div>
      ) : null}
