...
import { FileText, Pencil, Plus, Save, CheckCircle2, X } from 'lucide-react';
import { apiFetch, downloadFile } from '../../lib/api-client';

type Template = { id: string; name: string; content: string; isDefault: boolean };
type Acknowledgement = { id: string; status: 'PENDING' | 'ACKNOWLEDGED'; templateName: string; generatedAt: string; generatedByUserId: string; acknowledgedAt?: string; acknowledgedByUserId?: string; acknowledgementNote?: string };
...
export default function AssetAcknowledgementPanel({ assetId }: { assetId: string }) {
  ...
  const [latest, setLatest] = useState<Acknowledgement | null>(null);
  ...
  async function load() {
    try {
      const [rows, acknowledgement] = await Promise.all([
        apiFetch('/asset-acknowledgements/templates'),
        apiFetch(`/asset-acknowledgements/assets/${assetId}`),
      ]);
      const list = Array.isArray(rows) ? rows : [];
      setTemplates(list);
      setLatest(acknowledgement ?? null);
  ...
  async function generate() {
    setBusy(true); setMessage(null);
    try {
      const result = await downloadFile(`/asset-acknowledgements/assets/${assetId}/pdf`, false);
      const url = URL.createObjectURL(result.blob); const a = document.createElement('a'); a.href = url; a.download = result.filename || `asset-acknowledgement-${assetId}.pdf`; a.click(); URL.revokeObjectURL(url);
      setMessage('Acknowledgement generated. Employee acknowledgement is now pending.');
      await load();
  ...
  async function acknowledge() {
    if (!latest || latest.status === 'ACKNOWLEDGED') return;
    setBusy(true); setMessage(null);
    try {
      await apiFetch(`/asset-acknowledgements/${latest.id}/acknowledge`, { method: 'POST', body: JSON.stringify({}) });
      setMessage('Asset acknowledgement recorded.');
      await load();
    } catch (e: any) { setMessage(e?.message ?? 'Only the assigned employee can acknowledge this asset.'); }
    finally { setBusy(false); }
  }
  ...
  return <section className='rounded-2xl border border-slate-200 bg-white p-6 shadow-sm'>
    <div className='flex flex-col gap-3 md:flex-row md:items-start md:justify-between'>
      ...
      <div className='flex flex-wrap gap-2'>
        <button onClick={() => setEditing(true)} className='ui-interactive inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold'><Pencil size={15}/>Edit content</button>
        <button onClick={() => void generate()} disabled={busy} className='ui-interactive inline-flex items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-sm font-semibold text-white'><FileText size={15}/>{busy ? 'Working…' : 'Generate PDF'}</button>
        {latest?.status === 'PENDING' && <button onClick={() => void acknowledge()} disabled={busy} className='ui-interactive inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700'><CheckCircle2 size={15}/>Acknowledge</button>}
      </div>
    </div>
    {latest && <div className={`mt-4 rounded-xl border px-4 py-3 text-sm ${latest.status === 'ACKNOWLEDGED' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
      <div className='font-semibold'>{latest.status === 'ACKNOWLEDGED' ? 'Acknowledged' : 'Pending employee acknowledgement'}</div>
      <div className='mt-1 text-xs opacity-80'>Generated {new Date(latest.generatedAt).toLocaleString()} · Template: {latest.templateName}{latest.acknowledgedAt ? ` · Acknowledged ${new Date(latest.acknowledgedAt).toLocaleString()}` : ''}</div>
    </div>}
