...
export default function AssetAcknowledgementPanel({ assetId }: { assetId: string }) {
  ...
  async function load() { try { const rows = await apiFetch('/asset-acknowledgements/templates'); const list = Array.isArray(rows) ? rows : []; setTemplates(list); const current = list.find((t: Template) => t.isDefault) ?? list[0]; if (current) { setSelected(current); setName(current.name); setContent(current.content); } } catch { setMessage('Unable to load acknowledgement templates.'); } }
  useEffect(() => { void load(); }, []);
