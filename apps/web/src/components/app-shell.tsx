...
import { LayoutDashboard, Laptop, Building2, Users, ShieldCheck, Settings, FileClock, KeyRound, Boxes, MapPin, Truck, ClipboardCheck } from 'lucide-react';
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const context = <div className="rounded-xl bg-white/5 px-3 py-2.5"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Access level</div><div className="mt-1 text-sm font-semibold text-white">{levelLabel(adminLevel)}</div></div>;
  const footer = <Link href="/settings" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-xl p-3 text-sm hover:bg-white/5 hover:text-white"><Settings size={17} aria-hidden="true" />Settings</Link>;

  return <div className="min-h-screen bg-slate-50"><Sidebar groups={visibleGroups} variant="tenant" mobileOpen={mobileOpen} onMobileClose={() => setMobileOpen(false)} brand={brand} context={context} footer={footer} ariaLabel="Tenant navigation" /><div className="relative min-h-screen lg:pl-64"><TenantBackground /><header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:pl-4"><button onClick={() => setMobileOpen(true)} aria-label="Open navigation" className="rounded-xl p-2 hover:bg-[var(--theme-primary-soft)] lg:hidden">☰</button><div className="min-w-0 flex-1"><div className="hidden text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 sm:block">AssetHub Tenant Console</div><div className="truncate text-sm font-semibold text-slate-900">{displayName}</div></div><div className="flex items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--theme-primary-soft)] text-xs font-bold text-[var(--theme-link)]">{initials(displayName)}</span><div className="hidden sm:block"><div className="text-xs font-bold text-slate-900">{levelLabel(adminLevel)}</div><div className="text-[11px] text-slate-500">{tenantProfile?.primaryEmail ?? 'Signed in'}</div></div></div></header><main className="relative z-10 mx-auto max-w-[1600px] bg-transparent p-4 sm:p-6 lg:p-8">{children}</main></div></div>;
}
