...
export default function AppShell({children}:{children:ReactNode}){
 const pathname=usePathname();const{adminLevel,tenantProfile}=useAuth();const[open,setOpen]=useState(false);const displayName=tenantProfile?.name??'Tenant';
 ...
 const renderNav=()=> <aside className="flex h-full flex-col bg-[var(--theme-sidebar)] text-[var(--theme-sidebar-text)]"><div className="flex h-16 items-center border-b border-white/10 px-4"><Link href="/dashboard" aria-label="AssetHub dashboard" className="block w-[190px]"><img src="/assethub-logo-dark.svg" alt="AssetHub" className="h-auto w-full"/></Link></div><div className="border-b border-white/10 px-4 py-3"><div className="rounded-xl bg-white/5 px-3 py-2.5"><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Access level</div><div className="mt-1 text-sm font-semibold text-white">{levelLabel(adminLevel)}</div></div></div><nav className="flex-1 overflow-auto px-3 py-4">{visibleGroups.map(group=><section key={group.label} className="mb-5 last:mb-0"><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-widest text-slate-500">{group.label}</p><div className="space-y-1">{group.items.map(({href,label,Icon})=>{const active=pathname===href||pathname.startsWith(`${href}/`);return <Link key={href} href={href} onClick={()=>setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active?'bg-[var(--theme-sidebar-active)] text-white shadow-sm':'hover:bg-[var(--theme-sidebar-hover)] hover:text-white'}`}><Icon size={17} aria-hidden="true"/><span>{label}</span></Link>})}</div></section>)}</nav><Link href="/settings" onClick={()=>setOpen(false)} className="flex items-center gap-3 border-t border-white/10 p-4 text-sm hover:text-white"><Settings size={17} aria-hidden="true"/>Settings</Link></aside>;
 return <div className="min-h-screen bg-slate-50"><div className="fixed inset-y-0 left-0 hidden w-64 lg:block">{renderNav()}</div>{open&&<div className="fixed inset-0 z-50 bg-slate-950/70 lg:hidden"><div className="h-full w-72">{renderNav()}</div></div>}<div className="relative min-h-screen lg:pl-64"><TenantBackground/><header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur"><button onClick={()=>setOpen(true)} aria-label="Open navigation" className="rounded-xl p-2 hover:bg-[var(--theme-primary-soft)] lg:hidden"><Menu size={20}/></button><div className="min-w-0 flex-1"><div className="hidden text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 sm:block">AssetHub Tenant Console</div><div className="truncate text-sm font-semibold text-slate-900">{displayName}</div></div><div className="flex items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--theme-primary-soft)] text-xs font-bold text-[var(--theme-link)]">{initials(displayName)}</span><div className="hidden sm:block"><div className="text-xs font-bold text-slate-900">{levelLabel(adminLevel)}</div><div className="text-[11px] text-slate-500">{tenantProfile?.primaryEmail??'Signed in'}</div></div></div></header><main className="relative z-10 mx-auto max-w-[1600px] bg-transparent p-4 sm:p-6 lg:p-8">{children}</main></div></div>;
}
