...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const renderNav = () => (
    <aside className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-[var(--theme-sidebar)] text-[var(--theme-sidebar-text)]">
      ...
      <nav aria-label="Tenant navigation" className="relative min-h-0 flex-1 overflow-y-scroll overscroll-contain px-3 py-4 [scrollbar-width:auto] [scrollbar-gutter:stable]">{visibleGroups.map((group) => <section key={group.label} className="mb-5 last:mb-0"><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-widest text-slate-500">{group.label}</p><div className="space-y-1">{group.items.map(({ href, label, Icon }) => { const active = pathname === href || pathname.startsWith(`${href}/`); return <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? 'bg-[var(--theme-sidebar-active)] text-white shadow-sm' : 'hover:bg-[var(--theme-sidebar-hover)] hover:text-white'}`}><Icon size={17} aria-hidden="true" /><span>{label}</span></Link>; })}</div></section>)}</nav>
