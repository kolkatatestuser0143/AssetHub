...
export default function AssetDocumentsPanel({ assetId }: { assetId: string }) {
  ...
  async function upload() {
    ...
    try {
      ...
      const data = await apiFetch(`/assets/${assetId}/documents?documentType=${encodeURIComponent(documentType)}`, { method: 'POST', body: form });
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-[180px_1fr_auto]">
        ...
        <input ref={inputRef} type="file" accept="application/pdf,image/jpeg,image/png,image/webp,text/plain,.doc,.docx,.xls,.xlsx" className="h-11 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" />
        <button type="button" onClick={() => void upload()} disabled={uploading} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"><Upload size={16} />{uploading ? 'Uploading…' : 'Upload'}</button>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {loading ? [1, 2, 3].map((item) => <div key={item} className="h-14 animate-pulse rounded-xl bg-slate-100" />) : documents.length === 0 ? (
          <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center"><FileText size={26} className="mx-auto text-slate-300" /><p className="mt-2 text-sm font-semibold text-slate-700">No documents attached</p><p className="mt-1 text-xs text-slate-500">Upload the first asset record above.</p></div>
