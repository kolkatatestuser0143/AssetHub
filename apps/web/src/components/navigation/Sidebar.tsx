...
export function Sidebar({ groups, variant = 'tenant', mobileOpen = false, onMobileClose, brand, context, footer, ariaLabel = 'Navigation' }: {
  ...
  const itemActive = dark
    ...
    : 'bg-[color-mix(in_srgb,var(--theme-primary-soft)_72%,var(--theme-sidebar))] text-[var(--theme-primary)] shadow-[0_4px_14px_color-mix(in_srgb,var(--theme-primary)_10%,transparent)] ring-1 ring-inset ring-[color-mix(in_srgb,var(--theme-primary)_20%,transparent)] before:absolute before:left-0 before:top-2 before:bottom-2 before:w-0.5 before:rounded-r-full before:bg-[var(--theme-primary)]';
  const itemIdle = dark
    ? 'text-slate-300 hover:bg-slate-900/90 hover:text-white hover:ring-1 hover:ring-inset hover:ring-slate-800'
    : 'text-[var(--theme-sidebar-text)] hover:bg-[color-mix(in_srgb,var(--theme-primary-soft)_34%,var(--theme-sidebar))] hover:text-[var(--theme-link)] hover:ring-1 hover:ring-inset hover:ring-[color-mix(in_srgb,var(--theme-primary)_10%,transparent)]';
