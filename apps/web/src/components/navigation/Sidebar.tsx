...
export function Sidebar({ groups, variant = 'tenant', mobileOpen = false, onMobileClose, brand, context, footer, ariaLabel = 'Navigation' }: {
  ...
  const content = <aside className={`flex h-screen min-h-0 w-full flex-col overflow-hidden shadow-2xl ${panel}`}>
    ...
    <nav aria-label={ariaLabel} className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 [scrollbar-gutter:stable]">
      {groups.map((group) => <section key={group.label} className="ui-sidebar-group mb-5 last:mb-0">
        ...
        <div className="space-y-1">
          {group.items.map((item) => {
            ...
            return <Link key={item.href} href={item.href} onClick={onMobileClose} className={`ui-sidebar-item motion-focus flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? itemActive : itemIdle}`}><Icon size={17}/><span className="min-w-0 flex-1">{item.label}</span></Link>;
          })}
        </div>
      </section>)}
    </nav>
