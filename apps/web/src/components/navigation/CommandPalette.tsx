...
import { Asset, Command, FileText, LayoutDashboard, Search, Settings, ShieldCheck, Users, X } from 'lucide-react';
  ...
  return <div className="fixed inset-0 z-[80] bg-slate-950/40 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Search AssetHub">
    <div className="mx-auto mt-[10vh] w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="max-h-[60vh] overflow-y-auto p-2">
        ...
        {matches.nav.map(([href, label, Icon]) => <button key={href} onClick={() => go(href)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm hover:bg-slate-50 ${pathname === href ? 'bg-slate-50 font-semibold' : ''}`}><Icon size={17} className="text-[var(--theme-link)]"/><span className="flex-1">{label}</span><span className="text-xs text-slate-400">Enter</span></button>)}
        {matches.assets.length > 0 && <><p className="mt-2 px-3 py-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400">Assets</p>{matches.assets.map((asset) => <Link key={asset.id} href={`/assets/${asset.id}`} onClick={() => { setOpen(false); setQuery(''); }} className="flex items-center gap-3 rounded-xl px-3 py-3 hover:bg-slate-50"><Asset size={17} className="text-[var(--theme-link)]"/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-900">{asset.assetNumber}</span><span className="block text-xs text-slate-500">{asset.assetType?.name ?? 'Asset'} · {asset.status}</span></span></Link>)}</>}
